...
把飞书 / Lark 消息和本地 Codex CLI 打通的 Codex-first 轻量 bot，同时保留 Claude Code 支持。用一条命令起服务，扫码绑应用，在飞书里和 agent 对话、让它读图 / 改代码。
...
- **会话延续**：每个 chat 独立 agent session，对话能接着上次说
...
这个项目可以直接从 GitHub 运行，不需要发布到 npm。把 `<your-github-user>` 换成你的 GitHub 用户名或组织名。
...
4. 成功后凭据写入 `~/.lark-codex-bridge/config.json`
...
lark-codex-bridge ps                      列出本机所有正在跑的 bridge 进程
lark-codex-bridge kill <id|#>             kill 指定 bridge 进程（SIGTERM，2s 后 SIGKILL）
lark-codex-bridge --help                  列所有命令
```

**服务层**（让 OS 在后台托管 bridge）:

> ⚠️ **服务层命令必须先全局安装,不能直接用 npx**。daemon 的 launchd plist / systemd unit / Windows 任务里会**硬编码** bridge CLI 的路径;通过 `npx ... start` 调用时,这条路径在 npm 的临时缓存里(`~/.npm/_npx/<hash>/...`),会被 GC 清掉。请先 `npm install -g github:<your-github-user>/lark-codex-bridge`,再 `lark-codex-bridge start`。`run` 用 npx 调用没问题(单次前台进程)。

```
lark-codex-bridge start                   注册（如需）+ 启动后台 daemon
lark-codex-bridge stop                    停止 daemon 并关闭开机自启
lark-codex-bridge restart                 重启 daemon
lark-codex-bridge status                  查看 daemon 状态（pid、日志路径、上次退出码）
lark-codex-bridge unregister              撤销注册（停止 + 删除服务定义文件）
```

daemon 崩溃会被自动拉起，用户登录时也会自动启动。平台映射:
- **macOS** → `launchd` 用户代理 `~/Library/LaunchAgents/ai.lark-codex-bridge.bot.plist`
- **Linux** → `systemd` 用户单元 `~/.config/systemd/user/lark-codex-bridge.bot.service`。要让 daemon 在退出登录后还能跑，执行一次 `loginctl enable-linger $USER`。
- **Windows** → Task Scheduler 任务 `LarkCodexBridge.Bot`，触发条件为 ONLOGON。启动脚本位于 `~/.lark-codex-bridge/daemon-launcher.cmd`。

daemon 的 stdout / stderr 写到 `~/.lark-codex-bridge/logs/daemon-stdout.log` 和 `daemon-stderr.log`，跟 bridge 自己的每日结构化日志放在同一个目录。
...
| `/config` | 调整偏好（agent、消息回复方式、工具调用显示等） |
...
**Agent 选择**：`/config` 可以选择 `自动`、`Codex CLI` 或 `Claude Code`。`自动` 会优先用 Codex，不可用时回退到 Claude。无头/服务部署时，也可以在 `~/.lark-codex-bridge/config.json` 里把 `preferences.agent` 设为 `"auto"`、`"codex"` 或 `"claude"`，或通过环境变量 `LARK_CODEX_BRIDGE_AGENT=codex` 覆盖。

**和原 Claude bridge 同机运行**：本项目默认把运行状态放在 `~/.lark-codex-bridge/`，不会读取或覆盖原 bridge 的 `~/.lark-channel/`。两个 bridge 请分别使用不同的飞书/Lark PersonalAgent 应用；同一个应用同时开两条长连接时，开放平台不会稳定地把消息发给指定进程。
...
| 路径 | 内容 |
|---|---|
| `~/.lark-codex-bridge/config.json` | 应用凭据（App ID / Secret），权限 600 |
| `~/.lark-codex-bridge/sessions.json` | 每个 chat / 话题 的 agent session id + cwd（+ 可选的 `/timeout` 覆盖） |
| `~/.lark-codex-bridge/workspaces.json` | 工作空间映射 |
| `~/.lark-codex-bridge/processes.json` | 当前在跑的 start 进程注册中心（`ps`/`stop` 用），死进程会被自动清理 |
| `~/.lark-codex-bridge/media/<chatId>/` | 下载的图片 / 文件，24h 自动清理 |
| `~/.lark-codex-bridge/logs/YYYY-MM-DD.log` | 结构化运行日志（JSON line），按天滚动；启动时清理超过 7 天的老文件（`LARK_CODEX_BRIDGE_LOG_DAYS` 环境变量可改）；`/doctor` 命令读它做诊断 |

> 升级自原 bridge 0.1.11 之前的版本？只有在你明确想把旧状态导入 Codex bridge 时，才跑一次 `lark-codex-bridge migrate`。默认情况下，本项目独立使用 `~/.lark-codex-bridge/`。

## 隐私

不要提交 `~/.lark-codex-bridge/`、`~/.lark-channel/`、`.env`、`.npmrc`、日志、App ID、App Secret、`open_id` 或 `chat_id`。运行时凭据保存在仓库外，App Secret 会加密落盘。
...
不太想登飞书也可以，`/config` 表单背后写的是 `~/.lark-codex-bridge/config.json` 的 `preferences.access`：
...
**agent 挂住不回复**：通常是选中的 CLI 没登录、没安装，或者 session 指向了不存在的 cwd。发 `/status` 看当前状态；`/new` 重开会话往往就好。
