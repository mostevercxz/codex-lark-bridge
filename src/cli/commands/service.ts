...
/**
 * Poll `~/.lark-codex-bridge/processes.json` for a freshly-registered bridge
...
async function reportConnectAfter(
  ...
  if (entry) {
    const agent = isComplete(cfg) ? new ConfiguredAgentAdapter(() => cfg) : undefined;
 ...
 * definition file.
 *
 * Idempotent. Leaves ~/.lark-codex-bridge/ state untouched (keystore, sessions,
...
export async function runServiceUnregister(): Promise<void> {
  ...
  console.log('  (配置 / 日志 / 会话保留在 ~/.lark-codex-bridge/)');
}
