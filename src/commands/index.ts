...
async function handleResume(args: string, ctx: CommandContext): Promise<void> {
  ...
  const sessions = await listRecentSessions(cwd, limit, ctx.agent.id);
  ...
  const entries = sessions.map((s) => ({
    ...
    current:
      ...
      (currentSession.agentId ?? 'claude') === ctx.agent.id,
  }));
...
async function handleStatus(_args: string, ctx: CommandContext): Promise<void> {
  ...
  const sessionAgent = sess?.agentId ?? 'claude';
  const card = statusCard({
    cwd,
    sessionId: sess?.sessionId,
    sessionStale: Boolean(sess && (sess.cwd !== cwd || sessionAgent !== ctx.agent.id)),
...
- phase: 模块阶段。常见值: ws(WebSocket), intake(消息入站), queue(去抖队列), flush(批处理), media(附件下载), prompt(prompt 组装), session(会话), agent(本地 agent 子进程), card(卡片渲染), comment(文档评论), cardAction(卡片回调), command(斜杠命令), sdk(飞书 SDK 内部)
...
async function submitAccount(ctx: CommandContext): Promise<void> {
  ...
  void (async () => {
    ...
    // raw secret. External tooling can read the same SecretRef and go through
    // the exec protocol to retrieve the plaintext without keeping plaintext
    // on disk.
...
async function showConfigForm(ctx: CommandContext): Promise<void> {
  ...
  const card = configFormCard({
    agent: getAgentPreference(ctx.controls.cfg),
...
async function submitConfig(ctx: CommandContext): Promise<void> {
  ...
  const agent =
    rawAgent === 'claude' || rawAgent === 'codex' || rawAgent === 'auto'
      ? rawAgent
      : getAgentPreference(ctx.controls.cfg);
